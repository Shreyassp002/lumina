                  ...
                  <div className="space-y-4">
                    {userListings.length === 0 && userAuctions.length === 0 ? (
                      <div className="text-center py-12">
                        ...
                        <p className="text-gray-600">Your listings and auctions will appear here</p>
                      </div>
                    ) : (
                      <>
                        {userListings.map((l) => (
                          <div key={`listing-${l.listingId}`} className="flex items-center p-4 bg-gray-50 rounded-lg">
                            <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center mr-4">
                              <User className="w-5 h-5 text-blue-600" />
                            </div>
                            <div className="flex-1">
                              <p className="text-sm font-medium text-gray-900">Listed NFT #{l.tokenId}</p>
                              <p className="text-sm text-gray-600">Price: {Number(l.price) / 1e18} ETH</p>
                            </div>
                          </div>
                        ))}
                        {userAuctions.map((a) => (
                          <div key={`auction-${a.id}`} className="flex items-center p-4 bg-gray-50 rounded-lg">
                            <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center mr-4">
                              <TrendingUp className="w-5 h-5 text-green-600" />
                            </div>
                            <div className="flex-1">
                              <p className="text-sm font-medium text-gray-900">Auctioning NFT #{a.tokenId}</p>
                              <p className="text-sm text-gray-600">Ends at {new Date(a.endTime).toLocaleString()}</p>
                            </div>
                          </div>
                        ))}
                      </>
