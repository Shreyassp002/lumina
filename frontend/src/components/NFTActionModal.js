...
import { useMarketplaceApproval, useAuctionApproval, useApproveMarketplace, useApproveAuction } from '../hooks/useApproval';
import { X, DollarSign, Clock, Gavel, Zap, Shield, AlertCircle } from 'lucide-react';
...
export default function NFTActionModal({
    ...
    // Check approvals
    const { data: marketplaceApproval } = useMarketplaceApproval(nft?.tokenId, address);
    const { data: auctionApproval } = useAuctionApproval(nft?.tokenId, address);
    const { approveMarketplace, isPending: isApprovingMarketplace } = useApproveMarketplace();
    ...
    const handleSubmit = (e) => {
        ...
        if (action === 'list') {
            onList(nft.tokenId, BigInt(Math.floor(parseFloat(formData.price) * 1e18)));
        } else if (action === 'auction') {
            const minIncrement = formData.minIncrement || (parseFloat(formData.startPrice) * 0.1);
            onAuction(
                nft.tokenId,
                ...
                0, // English auction
                formData.buyNowPrice ? BigInt(Math.floor(parseFloat(formData.buyNowPrice) * 1e18)) : null
            );
        }
    };
                        ...
                        <div className="flex items-start space-x-3">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-sm text-yellow-200 mt-1">
                                    You need to approve the {action === 'list' ? 'marketplace' : 'auction'} contract to transfer this NFT.
                                </p>
                                <button
                                    type="button"
                                ...
                                >
                                    ...
                                    {(isApprovingMarketplace || isApprovingAuction) ? 'Approving...' : 'Approve Contract'}
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    {action === 'list' ? (
                        ...
                        <>
                            <div>
                                ...
                                {errors.startPrice && (
                                    <p className="text-red-400 text-sm mt-1">{errors.startPrice}</p>
                                )}
                            </div>
                            ...
                            <div>
                                ...
                                {errors.buyNowPrice && (
                                    <p className="text-red-400 text-sm mt-1">{errors.buyNowPrice}</p>
                                )}
                                <p className="text-sm text-green-200/70 mt-1">
