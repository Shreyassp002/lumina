  ...
  return (
    <div className="glass-panel p-6 rounded-lg">
      ...
      <button
        onClick={check}
        disabled={loading}
        className="mb-4 px-4 py-2 bg-gradient-to-r from-emerald-500 to-lime-500 text-black rounded-lg hover:from-emerald-400 hover:to-lime-400 disabled:opacity-50"
      >
      ...
      {info && (
        <div className="space-y-2 text-sm">
          {info.error ? (
            ...
            <>
              <div>
                <span className="text-emerald-300">Timestamp:</span>{" "}
                {info.timestamp}
              </div>
              <div>
                <span className="text-emerald-300">Current Auction ID:</span>{" "}
                {info.currentAuctionId}
              </div>
              <div className="mt-2">
                <span className="text-emerald-300">Auctions:</span>
                <pre className="mt-2 p-2 bg-[#0e1518] rounded text-xs overflow-auto">
                  {JSON.stringify(info.auctions, null, 2)}
                </pre>
              </div>
            </>
          )}
